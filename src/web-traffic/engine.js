...
const log = makeLogger('WebTraffic');
...
async function runCampaign(campaignId) {
  const db = getDb();
  const campaign = db.prepare('SELECT * FROM web_campaigns WHERE id = ?').get(campaignId);
  if (!campaign || campaign.status !== 'running') return;

  const proxies = db.prepare("SELECT * FROM proxies WHERE status = 'active'").all();
  if (!proxies.length) {
    db.prepare("UPDATE web_campaigns SET status = 'failed' WHERE id = ?").run(campaignId);
    log.info('No active proxies', { campaignId });
    return;
  }

  log.info(`Starting campaign "${campaign.name}"`, {
    campaignId,
  ...
  while (i < remaining) {
    const current = db.prepare('SELECT status FROM web_campaigns WHERE id = ?').get(campaignId);
    if (current.status !== 'running') {
      log.info(`Campaign paused/cancelled: ${current.status}`, { campaignId });
      break;
    }
    ...
    for (const s of settled) {
      ...
      if (r.success) {
        db.prepare('UPDATE web_campaigns SET visits_sent = visits_sent + 1 WHERE id = ?').run(campaignId);
        db.prepare(`
          INSERT INTO web_visits (campaign_id, proxy_id, status, duration, pages, persona, device, user_agent)
          VALUES (?, ?, 'sent', ?, ?, ?, ?, ?)
        ...
        log.info(`Visit ok: ${r.device} | ${r.persona} | ${r.pages}p | ${r.duration}s | proxy#${proxy?.id}`, { campaignId });
      } else {
        db.prepare('UPDATE web_campaigns SET visits_failed = visits_failed + 1 WHERE id = ?').run(campaignId);
        db.prepare(`INSERT INTO web_visits (campaign_id, proxy_id, status) VALUES (?, ?, 'failed')`).run(campaignId, proxy?.id);
        log.info(`Visit failed: ${r.reason?.slice(0, 80)}`, { campaignId });
      }
    }
    ...
    const delayMs = rand(5000, 15000);
    log.info(`Next batch in ${(delayMs / 1000).toFixed(0)}s`, { campaignId });
    await sleep(delayMs);
  }

  const final = db.prepare('SELECT visits_sent, visits_total FROM web_campaigns WHERE id = ?').get(campaignId);
  const newStatus = final.visits_sent >= final.visits_total ? 'completed' : 'failed';
  db.prepare('UPDATE web_campaigns SET status = ?, completed_at = CURRENT_TIMESTAMP WHERE id = ?').run(newStatus, campaignId);
  log.info(`Campaign "${campaign.name}" ${newStatus} — ${final.visits_sent}/${final.visits_total}`, { campaignId });
}

module.exports = { runVisit, runCampaign };
